#' Cross-validation for ridge_fit
#'
#' @param x input matrix as in ridge_fit
...
#' lambda_values <- seq(0.1, 0.4, by = 0.1) # set up lambda values for cross-validation
#' fit <- ridge_cv(x, y, lambda = lambda_values, nfolds = 10)
#' fit$lambda.best
ridge_cv <- function(x, y, lambda = NULL, nfolds = 10) {
  ### prepare and check for all arguments
  if (!is.null(lambda) && length(lambda) < 2)stop("Need more than one value of lambda for ridge_cv")
  ...
  for (i in seq_along(lambda)) {
    ...
    for (j in seq(nfolds)) {
      ...
      if (stats::var(y_train) == 0) {
        ...
        model <- ridge_fit(x_train, y_train, lambda = current_lambda)

        # make predictions on validation set
        y_pred <- ridge_predict(model, x_val)
  ...
  model <- ridge_fit(x, y, lambda = lambda.best)
