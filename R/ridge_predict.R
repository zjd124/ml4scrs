#' make predictions from a "ridge_fit" object
#'
#' @param model fitted "ridge_fit" model object
...
#' fit <- ridge_fit(x,y,lambda = 0.01)
#' ridge_predict(fit, newdata = x[1:10, ])
ridge_predict <- function(model, newdata, lambda = NULL) {
  ...
  if (!inherits(model, "list") || !("beta" %in% names(model))) {
    stop("Invalid 'model'. Must be the result of ridge_fit.")
  }
  beta <- model$beta
