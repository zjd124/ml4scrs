...
#' fit <- lasso_fit(x,y,lambda = c(0.01, 0.02))
#' fit$beta
lasso_fit <- function(x, y, lambda = NULL) {
  ...
  for (i in seq_along(lambda)) {
    ...
    beta <- matrix(0, nrow = nvars + 1, ncol = 1)

    for (iteration in 1:10000) {
      h <- x %*% beta
      gradient <- (1/nobs) * t(x) %*% (h - y) + current_lambda * sign(beta)
      beta <- beta - gradient
      # apply soft-thresholding to shrink coefficients towards zero
      beta <- sign(beta) * pmax(abs(beta) - current_lambda, 0)
    }
    betas[, i] <- beta
  }
